...
#define FILENAME "winning_at_sports"
...
{ 
    ...
    forj(b + 1)
        fori1(a + 1)
            if (i > j)
                table[i][j] = (table[i][j] + table[i - 1][j] + (j ? table[i][j - 1] : 0)) % INF;
    forj(b + 1)
        fori1(b + 1)
            if (i >= j)
                table2[i][j] = (table2[i][j] + table2[i - 1][j] + (j ? table2[i][j - 1] : 0)) % INF;
    writeln(table[a][b], table2[b][b]);
}
